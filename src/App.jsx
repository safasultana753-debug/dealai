            ...
            {tab === "actions" && brief && (
              <>
                ...
                {brief.riskFactors?.length > 0 && (
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#6B7A99", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 9 }}>⚠️ Risk Factors</div>
      ...
      <div className="auth-right">
        ...
        {mode === "forgot" && (
          <>
            ...
            {err && <div style={{ background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.25)", borderRadius: 8, padding: "9px 12px", fontSize: 12.5, color: "#EF4444", marginBottom: 12 }}>⚠️ {err}</div>}
  ...
  return (
    <div className="frappe-wrap">
      <div className="frappe-card">
        ...
        {err && <div style={{ background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.25)", borderRadius: 8, padding: "9px 12px", fontSize: 12.5, color: "#EF4444", marginBottom: 13 }}>⚠️ {err}</div>}
...
function SignalFeed({ deals }) {
  ...
  const signals = [
    ...
    { icon: "⚠️", deal: "FinTech Co", score: 88, type: "Risk", text: "Competitor Stripe Atlas launched a directly competing product. Review your positioning with Tom.", time: "2d ago" },
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
        {filters.map(f => (
          <button key={f} className="btn btn-xs" onClick={() => setActiveFilter(f)} style={{ background: activeFilter === f ? "#3B7BFF" : "transparent", color: activeFilter === f ? "#fff" : "#6B7A99", border: activeFilter === f ? "none" : "1px solid #1E2A42" }}>
            {f === "Funding" ? "💰 " : f === "Hiring" ? "👥 " : f === "News" ? "📰 " : f === "Risk" ? "⚠️ " : f === "Social" ? "📣 " : ""}{f}
